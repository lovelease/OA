...
function addResponse(){	
	if(XMLHttpReq.readyState==4){//判断对象状态
		if(XMLHttpReq.status==200){//信息已经成功返回，开始处理信息
			...
			xiugai.alt="修改";			
			xiugai.onclick=function(){alterNews(idInfo);};
			del=document.createElement("img");
			del.className="alter";
			del.src="IMG/delete.png";
			del.alt="删除";			
			del.onclick=function(){deleteNews(idInfo);};	
...
function validate(){
	...
	if(cont!=""){
		if(cont.length<=333){
			sendRequest("addNews.do",addResponse,"content="+cont);
		}else{
			alert("输入的信息过长!");
		}
	}else{
...
function deleteNews(id){
	if(confirm("确定删除这条记录吗？")){
		sendRequest("deleteNews.do",deleteResponse,"id="+id);
	}
}
...
function valid(id){
	...
	if(cnt!=""){
		sendRequest("alterNews.do",alterResponse,"cnt="+cnt+"&id="+id);
	}else{
		alert("你输入的内容为空");
	}
}